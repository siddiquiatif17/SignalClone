...
export default function Home() {
  ...
  const { isConnected, subscribe, unsubscribe, sendEvent } = useWebSocket();
  ...
  // References to keep event listeners synchronized
  ...
  // Track real-time presence overrides { user_id: { is_online, last_seen } }
  ...
  const markConversationRead = async (convId: number, messageId: number) => {
    try {
      // Send REST post request (also emits receipt updates via DB service)
      await apiFetch(`/conversations/${convId}/read`, {
        method: "POST",
        body: JSON.stringify({ message_id: messageId }),
      });
      
      // Also emit via WS for real-time compliance
      sendEvent("read", {
        conversation_id: convId,
        message_id: messageId,
      });

      // Clear unread count locally in UI list immediately
  ...
  useEffect(() => {
    if (selectedConvId) {
      ...
      // Clear typing indicator for the newly opened chat
      setTypingUsers({});
    } else {
      setMessages([]);
    }
    
    // Clear typing input compose states
    setNewMessage("");
  ...
  useEffect(() => {
    ...
    // 2. Handle new message events
    const handleNewMessage = (event: any) => {
      const msg = event.message as MessageRead;
      
      // If it belongs to the active conversation, append it and mark as read
      ...
      // Reload conversation listings
      fetchConversations();
    };

    // 3. Handle receipt update events
    const handleReceiptUpdate = (event: any) => {
      console.log("📑 WS Receipt Update received:", event);
      ...
      setMessages((prev) =>
        prev.map((msg) => {
          ...
          // Check if this receipt is already recorded
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    sendTypingEvent(false); // Stop typing immediately on submit
  ...
  const getMessageReceiptStatus = (msg: MessageRead, conv: ConversationResponseRead) => {
    if (msg.sender_id !== user?.id) return null; // Only show for messages sent by self
    
    const otherParticipants = conv.participants.filter((p) => p.user_id !== user?.id);
    if (otherParticipants.length === 0) return "sent";

    // For group chats and 1:1, check receipts statuses
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-950 font-sans text-slate-100 antialiased">
      {/* LEFT SIDEBAR */}
      <div className="flex w-80 flex-shrink-0 flex-col border-r border-slate-800 bg-slate-900">
        {/* Sidebar Header */}
        <div className="flex items-center justify-between p-4 pb-2">
          <div className="flex items-center gap-3">
            {/* User Avatar */}
            <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-sm font-semibold text-white shadow-md shadow-blue-900/20 overflow-hidden">
              {user?.avatar_url ? (
          ...
          <div className="flex gap-1">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-800 text-slate-300 transition-all hover:bg-slate-700 hover:text-white active:scale-95"
            >
            ...
            <button
              ...
              onClick={() => alert("Group creation coming soon in a later step!")}
              className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-800 text-slate-300 transition-all hover:bg-slate-700 hover:text-white active:scale-95"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-4.5 w-4.5">
        ...
        {/* Search Bar */}
        <div className="p-3">
          <div className="relative flex items-center rounded-lg bg-slate-950 px-3 py-2 border border-slate-800 focus-within:border-blue-600 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-2">
          ...
          <div className="mb-2 flex justify-around text-xs border-b border-slate-800 pb-1">
            <button
              onClick={() => setActiveTab("chats")}
              className={`px-2 py-1 font-semibold transition-colors border-b-2 ${
                activeTab === "chats" ? "border-blue-500 text-blue-400" : "border-transparent text-slate-400 hover:text-slate-200"
              }`}
            >
              Chats
            </button>
          ...
          {filteredConversations.length === 0 ? (
            <div className="text-center py-12 text-xs text-slate-600">
              {conversations.length === 0 ? (
                <div>
                  <p className="font-semibold text-slate-500 mb-1">No chats yet</p>
                  <p className="text-[10px]">Click the "+" icon above to start chatting with your contacts.</p>
                </div>
              ) : (
            ...
            <div className="space-y-0.5">
              {filteredConversations.map((conv) => {
                ...
                return (
                  ...
                  >
                    {/* Avatar */}
                    <div className="relative h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-blue-400 font-bold overflow-hidden border border-slate-800">
                      {conv.avatar_url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          src={conv.avatar_url}
                    ...
                    <div className="min-w-0 flex-1 text-left">
                      <div className="flex justify-between items-baseline">
                        ...
                        <span className={`text-[10px] flex-shrink-0 ml-1 ${isSelected ? "text-blue-200" : "text-slate-500"}`}>
                          {lastMsg ? formatTime(lastMsg.created_at) : formatTime(conv.updated_at)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-0.5">
                        <p className={`text-[11px] truncate mr-2 ${isSelected ? "text-blue-100" : "text-slate-400"}`}>
        ...
        {/* Sidebar Footer */}
        <div className="flex items-center justify-between border-t border-slate-800 bg-slate-900/90 p-3.5">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={logoutUser}
              title="Log Out"
              className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-800 hover:text-red-400 active:scale-95 transition-all"
            >
            ...
            <button
              title="Settings"
              className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-800 hover:text-white active:scale-95 transition-all"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-5 w-5">
      ...
      <div className="flex flex-1 flex-col bg-slate-950">
        {selectedConv ? (
          <div className="flex flex-1 flex-col overflow-hidden">
            {/* Active Chat Header */}
            <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900 p-3.5 shadow-md z-10 flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-100">{selectedConv.name}</h3>
              ...
              <div className="flex gap-1 text-slate-400">
                <button title="Call (Coming Soon)" className="rounded-full p-2 hover:bg-slate-800 hover:text-white transition-colors">
                ...
                <button title="Video Call (Coming Soon)" className="rounded-full p-2 hover:bg-slate-800 hover:text-white transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-950 px-4 py-3 flex flex-col space-y-2">
              {messagesLoading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-slate-600">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10 mb-2 text-slate-700">
                      ...
                      <React.Fragment key={msg.id}>
                        {showDateDivider && (
                          <div className="flex justify-center my-4 select-none">
                            <span className="text-[10px] font-semibold bg-slate-900/60 border border-slate-800/80 text-slate-500 rounded-full px-3.5 py-1 backdrop-blur-sm uppercase tracking-wider">
                              {getFullMessageDateString(msg.created_at)}
                            </span>
                          </div>
                        )}
                        <div
                          className={`flex flex-col group max-w-[75%] p-3 rounded-2xl shadow-sm ${
                            isSelf
                              ? "bg-blue-600 text-white rounded-tr-none ml-auto text-right"
                              : "bg-slate-800 text-slate-100 rounded-tl-none mr-auto text-left"
                          }`}
                        >
            ...
            {typingText && (
              <div className="px-4 py-1.5 text-[10px] text-slate-400 bg-slate-950 flex items-center gap-2 select-none border-t border-slate-900/60">
            ...
            <form onSubmit={handleSendMessage} className="p-3 border-t border-slate-800 bg-slate-900/90 flex gap-2 items-center flex-shrink-0 z-10">
              <button
                type="button"
              ...
              <input
                ...
                className="flex-1 rounded-xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-xs text-slate-100 placeholder-slate-500 outline-none focus:border-blue-600 transition-colors"
              />
          ...
          <div className="flex flex-1 flex-col">
            ...
            <div className="flex flex-1 flex-col items-center justify-center p-6 text-center">
              ...
              <h3 className="text-xl font-bold tracking-tight text-slate-200">Welcome to Signal</h3>
              <p className="mt-2 max-w-sm text-sm text-slate-500">
                Select a conversation to start messaging. Your communication is secure.
              </p>

              {/* Encryption Badge */}
              <div className="mt-8 flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-950/20 px-3.5 py-1.5 text-xs text-blue-400 backdrop-blur-sm">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                ...
                <span className="font-semibold tracking-wide uppercase text-[10px]">End-to-End Encrypted</span>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Bottom Mock Footer */}
        <div className="p-4 text-center border-t border-slate-900 bg-slate-950">
